...
def do_simple_invk_6dof():
    ...
    with mujoco.viewer.launch_passive(m, data) as viewer:
        ...
        while viewer.is_running():
            step_start = time.time()
            q_target_pos = robot.inverse_kinematics(ee_target_pos=cube_pos, step=1.0, joint_name="moving_side", nb_dof=6)
            robot.set_target_pos(q_target_pos)
...
if __name__ == "__main__":
    ...
    if args.robot == "5dof":
        ...
        do_simple_invk_6dof()
    elif args.robot == "6dof_soarm":
        do_simple_invk_6dof_soarm()
